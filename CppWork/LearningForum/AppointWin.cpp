    ...
    ui(new Ui::AppointWin)
{
   ...
   /* int t=f.general.size();
    ...
    for(int i=0;i<t;i++){
        QString s=f.moderator[i]->get_username();
        if(f.moderator[i]->get_board()==0){
             ui->listWidget_1->addItem(s);
        }
        else if(f.moderator[i]->get_board()==1){
            ui->listWidget_2->addItem(s);
       }
        else if(f.moderator[i]->get_board()==2){
            ui->listWidget_3->addItem(s);
       }
        else if(f.moderator[i]->get_board()==3){
            ui->listWidget_4->addItem(s);
       }
        else if(f.moderator[i]->get_board()==4){
            ui->listWidget_5->addItem(s);
       }
        else if(f.moderator[i]->get_board()==5){
            ui->listWidget_6->addItem(s);
       }
        else if(f.moderator[i]->get_board()==6){
            ui->listWidget_7->addItem(s);
       }
        else if(f.moderator[i]->get_board()==7){
